...
def coin():
    coin_variations = ['орел', 'решка']
    return f'<p>{random.choice(coin_variations)}</p>'

@app.route("/")
def index():
    return f'<h1><a href="/random_fact">Посмотреть случайный факт!</a><h1> <p><a href="/secret_page">Бросить монету!<a></p>'
